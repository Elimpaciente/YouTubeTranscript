...
async function handleRequest(request) {
  ...
  try {
    ...
    let transcript = null
    if (isShort) {
      transcript = await getKomeTranscript(youtubeUrl)
    } else {
      try {
        const videoId = extractVideoId(youtubeUrl)
        transcript = await getYTToTextTranscript(videoId)
      } catch (error) {
        transcript = await getKomeTranscript(youtubeUrl)
      }
    }
...
async function getYTToTextTranscript(videoId) {
...
async function getKomeTranscript(youtubeUrl) {
  ...
  if (!response.ok) throw new Error('Kome API failed')
  const data = await response.json()
